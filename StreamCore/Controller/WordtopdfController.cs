...
namespace StreamCore.Controller
{
    ...
    public class WordtopdfController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("/api/shipping/DeleteContainers")]
        public async Task<IActionResult> DeleteContainers([FromBody] DeleteContainerRequest request)
        {
            try
            {
                if (request?.TransportBoxNos == null || request.TransportBoxNos.Count == 0)
                    return BadRequest(new { error = "请至少勾选一个货柜" });

                var result = await _wordtopdfService.DeleteContainers(request.TransportBoxNos);
                return Ok(new { success = result });
            }
            catch (Exception ex)
            {
                Console.WriteLine($"DeleteContainers error: {ex.Message}");
